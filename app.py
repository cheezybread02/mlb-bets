...
book_mapping = {
    ...
    "fanatics": "22",
    "fanduel": "1", "fd": "1",
    "hard rock": "33", "hard rock (oh)": "33",
    "thescore bet": "5", "thescore": "5", "the score": "5"
}

league_mapping = {
    "ALL": "0", "FIFA": "9", "LIGA MX": "12", "MLB": "1", "MLS": "10",
    "NBA": "5", "NCAAB": "6", "NCAAF": "3", "NCAAW": "8", "NFL": "2",
    "NHL": "4", "SERIE A": "11", "WNBA": "7"
}

unique_books = [
    ("DraftKings", "2"), ("FanDuel", "1"), ("Bet365", "21"),
    ("Hard Rock (OH)", "33"), ("BetMGM", "4"), ("Caesars", "3"),
    ("Fanatics", "22"), ("TheScore Bet", "5")
]
...
def is_today_game(raw_text):
    ...
    future_date_patterns = [
        "7/29", "07/29", "7/30", "07/30", "7/31", "07/31",
        "jul 29", "jul 30", "jul 31", "july 29", "july 30", "july 31",
        "aug", "8/"
    ]
    ...
    current_date_patterns = [
        "7/28", "07/28", "7-28", "07-28",
        "jul 28", "july 28", "28 jul", "28 july"
    ]
...
def are_same_game(g1, g2):
    try:
        ...
        if g1_clean == g2_clean: return True
        ...
        if len(g1_teams) == 2 and len(g2_teams) == 2:
            ...
            if (w1_t1 & w2_t1) and (w1_t2 & w2_t2): return True
            if (w1_t1 & w2_t2) and (w1_t2 & w2_t1): return True
    except Exception:
        pass
    return False
...
def is_player_prop(raw_text):
    lower = raw_text.lower()
    return "player" in lower or "to record" in lower or ("over" in lower and ("hits" in lower or "runs" in lower or "strikeouts" in lower or "bases" in lower or "rbis" in lower or "homers" in lower or "walks" in lower or "yards" in lower or "touchdowns" in lower or "points" in lower or "rebounds" in lower or "assists" in lower))

def is_pitcher_strikeouts(raw_text):
    lower = raw_text.lower()
    return "strikeout" in lower or "ks" in raw_text or ("so" in lower and "pitcher" in lower)


# --- STREAMLIT UI SIDEBAR CONFIGURATION ---
st.sidebar.header("⚙️ Configuration")

target_league_input = st.sidebar.selectbox("Target League", list(league_mapping.keys()), index=3, key="cfg_league")

# Dynamically filter market types based on selected sport/league
league_upper = target_league_input.upper()
if league_upper == "MLB":
    available_markets = [
        "Moneyline", "Run Line", "Total (Over/Under)", 
        "Player Home Runs", "Player Total Bases", "Player Hits", 
        "Player Pitching Strikeouts", "Player RBIs", "Player Runs", "Player Walks"
    ]
elif league_upper in ["NFL", "NCAAF"]:
    available_markets = [
        "Moneyline", "Point Spread", "Total (Over/Under)", 
        "Player Touchdowns", "Player Passing Yards", "Player Rushing Yards", "Player Receiving Yards"
    ]
    ...
    available_markets = [
        ...
        "Player Goals", "Player Assists", "Player Points", "Player Shots On Goal"
    ]
elif league_upper in ["FIFA", "LIGA MX", "MLS", "SERIE A"]:
    available_markets = [
        "Moneyline", "Goal Spread", "Total Goals", "Player Goals", "Player Shots On Target"
    ]
else: # ALL or others
    available_markets = [
        "Moneyline", "Spread / Run Line / Puck Line", "Total (Over/Under)", 
        "Player Home Runs", "Player Touchdowns", "Player Points", "Player Goals", "Player Pitching Strikeouts"
    ]

exclude_input = st.sidebar.text_input("Global Teams to Exclude (comma-separated)", "", key="cfg_exclude")
excluded_teams = [t.strip().lower() for t in exclude_input.split(",") if t.strip()]

include_input = st.sidebar.text_input("Global Teams to Strictly Include (comma-separated)", "", key="cfg_include")
included_teams = [t.strip().lower() for t in include_input.split(",") if t.strip()]
...
for book in selected_books:
    with st.sidebar.expander(f"⚙️ {book} Settings", expanded=False):
        legs = st.slider(f"Legs per Parlay", 1, 5, 2, key=f"{book}_legs")
        ...
        requires_under = st.checkbox(f"Requires 'Under'", value=False, key=f"{book}_under")
        
        has_prop = any("Player" in m for m in market_types)
        if has_prop and mainlines:
            st.warning("⚠️ You selected a Player Prop market but have 'Mainlines Only' checked. The scraper will automatically UNCHECK 'Mainlines Only' on the site to find these props.")
...
st.sidebar.markdown("---")
run_button = st.sidebar.button("🚀 Run Scraper & Find Parlays", key="cfg_run_button", use_container_width=True)
...
if run_button:
    ...
    for book_name in selected_books:
        ...
        final_mainlines = 'n' if cfg['has_prop'] else ('y' if cfg['mainlines'] else 'n')
    ...
    async def run_playwright_scraper():
        async with async_playwright() as p:
            ...
            for session in saved_sessions:
                ...
                # Set Sportsbook
                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_WebUserControl_FilterSportsbookSite_DropDownListSportsbookSite_All").select_option(session['book_val'])
                
                # Set League
                league_val = league_mapping.get(target_league.upper(), "0")
                league_locator = page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_WebUserControl_FilterLeague_DropDownListLeague")
                await league_locator.select_option(league_val)
                await league_locator.evaluate("element => element.dispatchEvent(new Event('change', { bubbles: true }))")
                ...
                if session['market_types'] and session['market_types'][0]:
                    ...
                    # Strip leading prefixes like "Player " for cleaner matching on the site if desired
                    market_query = selected_m.replace("Player ", "")

                if market_query:
                    try:
                        market_input = page.locator("input").filter(has=page.locator("xpath=//ancestor::tr[contains(., 'Market Name Contains')]//input")).first
                        if not await market_input.count():
                            market_input = page.locator("input[id*='MarketName'], input[id*='Market']").first
                        if await market_input.count():
                            await market_input.click()
                            await market_input.fill(market_query)
                            await market_input.evaluate("element => element.dispatchEvent(new Event('change', { bubbles: true }))")
                    except Exception:
                        pass

                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_TextBoxMinimumEVPercentage").click()
                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_TextBoxMinimumEVPercentage").fill("-99")
                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_WebUserControl_FilterDevigMethod_DropDownListDevigMethod").select_option("8")
                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_WebUserControl_FilterOddsProviderCount_TextBoxMinimumOddsProviderCount").click()
                await page.locator("#ContentPlaceHolderMain_ContentPlaceHolderRight_WebUserControl_FilterOddsProviderCount_TextBoxMinimumOddsProviderCount").fill("5")
                ...
                for i in range(total_rows):
                    ...
                    clean_line = " | ".join([line.strip() for line in raw_text.splitlines() if line.strip()])
                    lower_text = clean_line.lower()
                    
                    if target_league != "ALL" and target_league.lower() not in lower_text:
                        continue
                    if not is_today_game(clean_line):
                        continue
                    if any(ex_team in lower_text for ex_team in session['excluded_teams']):
                        continue
                    if session['included_teams'] and not any(inc_team in lower_text for inc_team in session['included_teams']):
                        continue
                    ...
                    if len(sub_parts) > 1:
                        game_candidate = sub_parts[1].split("Player")[0].split("Run Line")[0].split("Total")[0].split("Moneyline")[0].split("Spread")[0].strip()
                        if "@" in game_candidate or "vs" in game_candidate.lower():
                            game_id = game_candidate

                    if game_id == "Unknown Game":
                        matchup_match = re.search(r'([A-Za-z\s]+)\s*(?:@|vs\.?)\s*([A-Za-z\s]+)', clean_line, re.IGNORECASE)
                        if matchup_match:
                            full_matchup = matchup_match.group(0)
                            words = full_matchup.split()
                            generic_words = ['baseball', 'football', 'basketball', 'hockey', 'soccer', target_league.lower(), 'today', 'tomorrow', 'at', 'vs']
                            clean_words = [w for w in words if w.lower() not in generic_words]
    ...
    scraped_data_per_session, all_scraped_games = loop.run_until_complete(run_playwright_scraper())
    loop.close()
    ...
    for s_idx, session in enumerate(saved_sessions):
        ...
        for combo in itertools.combinations(app_plays, session['legs']):
            ...
            if not is_valid_combo:
                continue
            if session['requires_under'] and not any(leg['is_under'] for leg in combo):
                continue
            ...
            distance_from_range = (min_dec_target - combined_dec) if combined_dec < min_dec_target else (combined_dec - max_dec_target if combined_dec > max_dec_target else 0.0)
    ...
    all_global_parlays.sort(key=lambda x: (not x['in_range'], x['distance_from_range'], -x['boosted_ev']))
    ...
    for parlay in all_global_parlays:
        ...
        for leg in parlay['legs']:
            ...
            matched_key = next((eg for eg in global_used_games if are_same_game(eg, g)), None)
            if matched_key:
                global_used_games[matched_key]['has_exclusive'] = global_used_games[matched_key]['has_exclusive'] or has_exc
            else:
    ...
    for s_idx, session in enumerate(saved_sessions):
        ...
        st.subheader(f"Results for {session['book_name'].upper()} ({session['target_league']})")
        for p_idx, parlay in enumerate(picks, 1):
            t_parlay_str = f"+{parlay['total_true_american']}" if parlay['total_true_american'] > 0 else str(parlay['total_true_american'])
            boost_str = f" | Boosted Odds: +{parlay['boosted_american']}" if session['boost'] > 0 else ""
            box_class = "parlay-box" if parlay['in_range'] else "parlay-box-out"
            header_class = "parlay-header" if parlay['in_range'] else "parlay-header-out"
            ...
            p_win = 1.0 / parlay['combined_dec'] if parlay['combined_dec'] > 0 else 0.5

            all_bet_details.append({'stake': 10.0, 'net_profit': net_profit_win, 'boosted_dec': parlay['boosted_dec'], 'p_win': p_win})
            ...
            """
            for leg in parlay['legs']:
                t_odd_str = f"+{leg['true_american']}" if leg['true_american'] > 0 else str(leg['true_american'])
                html_card += f'<div class="parlay-leg">• {leg["raw"]} <span style="color: #888;">[True Odds: {t_odd_str}]</span></div>'
            html_card += "</div>"
    ...
    if len(all_bet_details) > 0:
        ...
        for i in range(total_outcomes):
            ...
            for idx, w in enumerate(win_mask):
                if w: profit += all_bet_details[idx]['stake'] + all_bet_details[idx]['net_profit']
            if profit >= 0:
                winning_outcomes += 1

        breakeven_pct = (winning_outcomes / total_outcomes) * 100.0
        roi_percentage = (total_mass_expected_profit / total_stakes) * 100.0 if total_stakes > 0 else 0.0
